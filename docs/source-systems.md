...
|---|---|---|---|
| **accounts** | one row per company | `account_id` (PK), `account_name`, `industry`, `billing_address`, `tax_id`, `duns_number`, `website`, `owner_user_id`, `territory_id`, `parent_account_id` | Hierarchies via `parent_account_id`; ownership & territory reassigned over time |
