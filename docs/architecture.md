...
|---|---|
| **Incremental discovery** | Tracks which files it has seen in a durable checkpoint; only new files are processed. Scales to millions of files via file-notification mode (on Azure, backed by **Event Grid + Queue Storage**). |
...
- Data resides in cloud object storage (**ADLS Gen2**) with blob soft-delete/versioning and geo-redundant replication (**GRS/RA-GRS**) per the org DR tier.
